...
class ProfiloActivity : Fragment(),ConsiglioItemClickListener {
    ...
    companion object {
        private const val TAG = "ArmadioActivity"
    ...
    override fun onCreateView(
        ...
        if (user != null) {
            ...
            raccoltaUtenti.document(user.uid)
                .get()
                .addOnSuccessListener { document ->
                    if (document.exists()) {
                        ...
                        val imageList = getImageList(user.uid)
            ...
            // Resto del codice per gestire le altre funzionalità
            ...
            sondaggioButton.setOnClickListener {
                val postList = getPostList(user.uid)
            ...
            consiglioButton.setOnClickListener {
                val consigliList = getConsigliList(user.uid)
            ...
            fab.setOnClickListener{
                val intent = Intent(requireContext(), CreaPostActivity::class.java)
                startActivity(intent)
                Toast.makeText(requireContext(),"Crea un nuovo post",Toast.LENGTH_SHORT).show()
            }
        }
        return view
    }
    ...
    private fun getPostList(username: String): List<Post> {
        ...
        val user = auth.currentUser // Ottieni l'utente corrente

        if (user != null) { // Verifica se l'utente è loggato
            val query = postsCollection.whereEqualTo("uid", user.uid) // Utilizza l'UID dell'utente corrente
            ...
            query.get()
                .addOnSuccessListener { querySnapshot ->
                    ...
                    // Aggiorna l'adapter con la lista dei post
                    postAdapter = PostAdapter(postsList)
                    recyclerView.adapter = postAdapter
                    postAdapter.notifyDataSetChanged()
                }
                .addOnFailureListener { exception ->
                    Log.e(UserAccountFragment.TAG, "Error getting posts: ${exception.message}")
                }
        }
    ...
    private fun getConsigliList(uid: String): List<Consiglio> {
        val firestore = FirebaseFirestore.getInstance()
        val consigliCollection = firestore.collection("consigli")
        val consigliList = mutableListOf<Consiglio>()
        // Query per ottenere i consigli creati dall'utente loggato (filtrati per UID)
        ...
        if(user!=null) {
            ...
            query.get()
                ...
                .addOnFailureListener { exception ->
                    Log.e(UserAccountFragment.TAG, "Error getting consigli: ${exception.message}")
                }
        }
